...
describe(Dealership) do
  ...
  describe(".clear") do
    it("empties out all of the saved dealerships") do
      Dealership.new({:name => "Bob's Used Cars"}).save()
      Dealership.clear()
      expect(Dealership.all()).to(eq([]))
    end
  end
  ...
  describe('#add_vehicle') do
    it('adds a new vehicle to a dealership') do
      test_dealership = Dealership.new(:name => "Tyler's sweet rides")
      test_vehicle = Vehicle.new(:model => "Corolla", :year => "1996", :color => "Lime Green")
